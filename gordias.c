...
enum ins {
    ...
    RET, JMP, JMP_T, JMP_F, CNS_CHR, CNS_I32, CNS_FLT, CNS_STR, LOAD_GLB,
...
char *ins_label[] = {
    ...
    "return", "jump", "jump if T", "jump if F", "const char", "const i32",
    "const float", "const string", "load global", "store global",
    "move", "print"
};
...
void Cpu_run(struct cpu *cpu)
{
    ...
    static void *dispatch_table[] = {
        ...
        &&do_flt_to_i32, &&do_call, &&do_ret, &&do_jmp, &&do_jmp_t, &&do_jmp_f,
    ...
    #define ARITH_I32(op) \
        do { \
            ...
            src0 = *cpu->ip++; \
            src1 = *cpu->ip++; \
            cpu->reg[dest] = cpu->reg[src0] op cpu->reg[src1]; \
        } while(0)
    ...
    do_i32_to_flt: do_flt_to_i32: do_call: do_ret: do_jmp: do_jmp_t: do_jmp_f:
